...
export default {
  ...
  execute: async (args: z.infer<typeof inputSchema>, context: any) => {
    ...
    if (!checkPermission(context.user, 'workflow.activate')) {
      return {
        content: [{
          type: 'text' as const,
          text: 'Access denied: Insufficient permissions for workflow.activate'
        }]
      };
    }
    ...
    try {
      ...
      // SECURITY: Audit log the activation
      ...
      return {
        content: [{
          type: 'text' as const,
          text: `❌ Error activating workflow: ${error.message}`
        }]
      };
    }
  }
